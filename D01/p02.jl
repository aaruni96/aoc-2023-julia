input = """nkzjrdqrmpztpqninetwofour1znnkd
...
numdict = Dict("one" => 1, "two" => 2, "three" => 3, "four" => 4, "five" => 5, "six" => 6, 
"seven" => 7, "eight" => 8, "nine" => 9)

function findsmallest(line, oldmin, oldnum, newnum)
    a = findfirst(newnum, line)
    if (typeof(a) == Nothing)
        return [oldmin, oldnum]
    end
    if a[1] < oldmin
        return [a[1], numdict[newnum]]
    end
    return [oldmin, oldnum]
end

function findlargest(line, oldmax, oldnum, newnum)
    a = findlast(newnum, line)
    if (typeof(a) == Nothing)
        return [oldmax, oldnum]
    end
    if a[1] > oldmax
        return [a[1], numdict[newnum]]
    end
...
for line in eachline(IOBuffer(input))
    oldnum = "zero"
    oldpos = length(line)+1
    for num in keys(numdict)
        oldpos, oldnum = findsmallest(line, oldpos, oldnum, num)
    end
    rfpos = oldpos
    rfnum = oldnum
    ...
    oldpos = 0
    for num in keys(numdict)
        oldpos, oldnum = findlargest(line, oldpos, oldnum, num)
    end
    rlpos = oldpos
    rlnum = oldnum
    ...
    fr = r"[0-9]"
    lr = r"[0-9][a-z]*$"
    fpos = match(fr, line).offset
    lpos = match(lr, line).offset
    if fpos < rfpos
        rfpos = fpos
        rfnum = parse(Int, match(fr, line).match)
    end

    if lpos > rlpos
        rlpos = lpos
        rlnum = parse(Int, match(lr, line).match[begin])
    end
    global sum = sum + (rfnum*10) + rlnum
end
